...
export class UpdateArticle extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (articleToEdit && this.props.articleToEdit !== articleToEdit) {
      const {
        title, body, description, isPrivate
      } = articleToEdit;
      const article = {
        rawTextValue: {
          title,
  ...
  render() {
    ...
    console.log(this.state.rawTextValue.body);
    return (
      <div>
        ...
        <section className={classnames('section', { 'show-butter': errorMessage || responseMessage })}>
          ...
          <div className='container publish'>
            <button className='publish__button btn btn--white btn--animated' onClick={this.publishHandler} disabled={!this.state.rawTextValue.body}>Update and Publish</button>
          </div>
