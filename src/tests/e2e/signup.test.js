...
describe('End-to-End for Sign up Page', () => {
  ...
  test(
    'should not sign up user when there is a validation error',
    async () => {
      const realUser = {
        ...user,
        username: 'dummyUser****'
  ...
  test(
    ...
    async () => {
      ...
      expect(html).toBe('Username must be at least 4 characters');
      ...
      expect(html).toBe('Username must be alphanumeric');
      return null;
    },
    timeout
  );
  ...
  test(
    ...
    async () => {
      ...
      await page.waitForSelector('.error-js');
      const html = await page.$eval('.error-js', e => e.innerHTML);
      expect(html).toBe('Email provided is not valid');
      return null;
    },
    timeout
  );
  ...
  test(
    ...
    async () => {
      ...
      await page.waitForSelector('.error-js');
      const html = await page.$eval('.error-js', e => e.innerHTML);
      expect(html).toBe('Password must be at least 8 characters');
    },
    timeout
  );
  ...
  test(
    ...
    async () => {
      ...
      await page.waitForSelector('.error-js');
      const html = await page.$eval('.error-js', e => e.innerHTML);
      expect(html).toBe('Confirm password is not equal to password');
    },
    timeout
  );
});
